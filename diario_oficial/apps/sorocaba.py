...
from ..crawlers import SorocabaCrawler
from ..runner import run_crawler


BASE_URL = "https://noticias.sorocaba.sp.gov.br/jornal/"
OUTPUT_DIR = Path("DO/Sorocaba")
LOG_DIR = Path("logs")
STATE_DIR = Path("state")
STATE_FILE = STATE_DIR / "edicoes_lidas_sorocaba.json"
LOG_FILE = LOG_DIR / "diario_oficial_sorocaba.log"
USER_AGENT = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Crawler do Jornal do Município de Sorocaba com notificações Telegram."
    )
    parser.add_argument(
        "--url",
        default=BASE_URL,
        help=f"URL inicial da página do jornal (padrão: {BASE_URL})",
    )
    parser.add_argument(
        "--term",
        action="append",
